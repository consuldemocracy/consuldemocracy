...
feature "Legislation" do
  ...
  context "process empty" do
    let(:process) { create(:legislation_process, :empty, end_date: Date.current - 1.days) }
  ...
  context "phases empty" do
    let(:process) { create(:legislation_process, end_date: Date.current - 1.days) }
  ...
  context "only debates exist" do
    ...
    let(:process) { create(:legislation_process, end_date: Date.current - 1.days) }
    let(:debate) { create(:legislation_question, process: process, title: "Question 1") }
    let(:debate_2) { create(:legislation_question, process: process, title: "Question 2") }
    ...
    before do
      ...
      create(:debate_comment, user: user, commentable_id: debate_2.id, body: "Answer 3")
      create(:debate_comment, user: user, commentable_id: debate_2.id, body: "Answer 4")
    end
