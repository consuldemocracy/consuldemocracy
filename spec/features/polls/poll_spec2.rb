...
feature 'Polls' do
  ...
  context 'Show' do
    ...
    context "require all questions be answered" do
      ...
      background do
        ...
        answer1 = create(:poll_question_answer, question: question2, title: 'Han Solo 2')
        answer2 = create(:poll_question_answer, question: question2, title: 'Chewbacca 2')
      ...
      scenario 'Questions must appear in correct format ' do        
       
        expect(page).to have_css(".require-answer", count: 2)
        expect(page).to have_css(".poll-question-answers", count: 2)
        expect(page).to have_css("input.js-question-answer", count: 4)
        expect(page).to have_css("label.js-question-answer", count: 4)
      end

      scenario 'Questions must appear in correct format ' do        
        # scenario 1 - trying to answer with user not 'verified' by our criterious
        # scenario 2 - trying to answer with user 'verified' by our criterious
        choose('Han Solo') # label text
        choose('Han Solo 2') # label text
        #save_and_open_page
        click_button "Send"
        save_and_open_page
        expect(page).to have_css(".require-answer", count: 2)
        expect(page).to have_css(".poll-question-answers", count: 2)
        expect(page).to have_css("input.js-question-answer", count: 4)
        expect(page).to have_css("label.js-question-answer", count: 2) #4
