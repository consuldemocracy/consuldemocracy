...
feature 'Admin spending proposals' do
  ...
  scenario "Priority" do
    ...
    nebraska   = create(:geozone)

    cali_some_votes  = create(:spending_proposal, valuators: [scarlett], geozone: california, cached_votes_up: 50)
    cali_least_votes = create(:spending_proposal, valuators: [scarlett], geozone: california, cached_votes_up: 25)
    cali_most_votes  = create(:spending_proposal, valuators: [scarlett], geozone: california, cached_votes_up: 75)

    new_york_some_votes  = create(:spending_proposal, valuators: [scarlett], geozone: new_york, cached_votes_up: 50)
    new_york_least_votes = create(:spending_proposal, valuators: [scarlett], geozone: new_york, cached_votes_up: 25)
    new_york_most_votes  = create(:spending_proposal, valuators: [scarlett], geozone: new_york, cached_votes_up: 75)

    washington_some_votes  = create(:spending_proposal, valuators: [isabel], geozone: washington, cached_votes_up: 50)
    washington_least_votes = create(:spending_proposal, valuators: [isabel], geozone: washington, cached_votes_up: 25)
    washington_most_votes  = create(:spending_proposal, valuators: [isabel], geozone: washington, cached_votes_up: 75)

    nebraska_some_votes  = create(:spending_proposal, valuators: [isabel], geozone: nebraska, cached_votes_up: 50)
    nebraska_least_votes = create(:spending_proposal, valuators: [isabel], geozone: nebraska, cached_votes_up: 25)
    nebraska_most_votes  = create(:spending_proposal, valuators: [isabel], geozone: nebraska, cached_votes_up: 75)

    visit admin_spending_proposals_path

    click_link "Prioridades"
    fill_in "proposals_to_evaluate_per_district", with: 2
    click_button 'Enviar'

    within("#valuator_#{scarlett.id}") do
      expect(page).to have_content scarlett.description_or_email
      expect(page).to have_css(".total-count", text: 4)

      expect(page).to have_content cali_most_votes.title
      expect(page).to have_content cali_some_votes.title
      expect(page).to have_content new_york_most_votes.title
      expect(page).to have_content new_york_some_votes.title

      expect(page).to_not have_content cali_least_votes.title
      expect(page).to_not have_content new_york_least_votes.title
    end

    within("#valuator_#{isabel.id}") do
      expect(page).to have_content isabel.description_or_email
      expect(page).to have_css(".total-count", text: 4)

      expect(page).to have_content washington_most_votes.title
      expect(page).to have_content washington_some_votes.title
      expect(page).to have_content nebraska_most_votes.title
      expect(page).to have_content nebraska_some_votes.title

      expect(page).to_not have_content washington_least_votes.title
      expect(page).to_not have_content nebraska_least_votes.title
    end
