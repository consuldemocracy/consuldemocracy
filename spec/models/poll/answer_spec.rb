...
describe Poll::Answer do
  ...
  describe "#save_and_record_voter_participation" do
    ...
    it "updates a poll_voter with user and poll data" do
      ...
      duplicate_answer = answer.question.find_or_initialize_user_answer(answer.author, "No")
      duplicate_answer.save_and_record_voter_participation

      expect(poll.reload.voters.size).to eq(1)

      voter = poll.voters.first
      expect(voter.document_number).to eq(duplicate_answer.author.document_number)
      expect(voter.poll_id).to eq(duplicate_answer.poll.id)
    end
