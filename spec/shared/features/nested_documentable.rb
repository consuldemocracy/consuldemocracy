shared_examples "nested documentable" do |login_as_name, documentable_factory_name, path, documentable_path_arguments, fill_resource_method_name, submit_button, documentable_success_notice|
  ...
  describe "at #{path}" do
    ...
    it "shows new document link when max documents allowed limit is not reached" do
    ...
    it "does not show new document link when documentable max documents allowed limit is reached", :js do
      login_as user_to_login
      visit send(path, arguments)
    ...
    it "does not show max documents warning when no documents added", :js do
      login_as user_to_login
      visit send(path, arguments)
    ...
    it "shows max documents warning when max documents allowed limit is reached", :js do
      login_as user_to_login
      visit send(path, arguments)
    ...
    it "hides max documents warning after any document removal", :js do
      login_as user_to_login
      visit send(path, arguments)
    ...
    it "updates nested document file name after choosing a file", :js do
      login_as user_to_login
      visit send(path, arguments)
    ...
    it "updates nested document file title with file name after choosing a file when no title defined", :js do
      login_as user_to_login
      visit send(path, arguments)
    ...
    it "does not update nested document file title with file name after choosing a file when title already defined", :js do
      login_as user_to_login
      visit send(path, arguments)
    ...
    it "updates loading bar style after valid file upload", :js do
      login_as user_to_login
      visit send(path, arguments)
    ...
    it "updates loading bar style after unvalid file upload", :js do
      login_as user_to_login
      visit send(path, arguments)
    ...
    it "updates document cached_attachment field after valid file upload", :js do
      login_as user_to_login
      visit send(path, arguments)
    ...
    it "does not update document cached_attachment field after unvalid file upload", :js do
      login_as user_to_login
      visit send(path, arguments)
    ...
    it "shows document errors after documentable submit with empty document fields", :js do
      login_as user_to_login
      visit send(path, arguments)
    ...
    it "deletes document after valid file upload and click on remove button", :js do
      login_as user_to_login
      visit send(path, arguments)
    ...
    it "shows successful notice when resource filled correctly without any nested documents", :js do
      login_as user_to_login
      visit send(path, arguments)
    ...
    it "shows successful notice when resource filled correctly and after valid file uploads", :js do
      login_as user_to_login
      visit send(path, arguments)
      send(fill_resource_method_name) if fill_resource_method_name
    ...
    it "shows new document after successful creation with one uploaded file", :js do
      login_as user_to_login
      visit send(path, arguments)
      send(fill_resource_method_name) if fill_resource_method_name
    ...
    it "shows resource with new document after successful creation with maximum allowed uploaded files", :js do
      login_as user_to_login
      visit send(path, arguments)
    ...
    if path.include? "edit"
      ...
      it "shows persisted documents and remove nested_field" do
        login_as user_to_login
      ...
      it "does not show add document button when documentable has reached maximum of documents allowed", :js do
        login_as user_to_login
      ...
      it "shows add document button after destroy one document", :js do
        login_as user_to_login
        create_list(:document, documentable.class.max_documents_allowed, documentable: documentable)
        visit send(path, arguments)
      ...
      it "removes nested field after remove document", :js do
        login_as user_to_login
        create(:document, documentable: documentable)
        visit send(path, arguments)
        click_on "Remove document"
