shared_examples "nested imageable" do |imageable_factory_name, path, imageable_path_arguments, fill_resource_method_name, submit_button, imageable_success_notice, has_many_images = false|
  ...
  describe "at #{path}" do
    ...
    it "shows new image link when imageable has not an associated image defined" do
      login_as user
    ...
    it "hides new image link after add one image" do
      login_as user
      visit send(path, arguments)
    ...
    it "updates nested image file name after choosing any file", :js do
      login_as user
      visit send(path, arguments)
    ...
    it "updates nested image file title with file name after choosing a file when no title defined", :js do
      login_as user
      visit send(path, arguments)
    ...
    it "does not update nested image file title with file name after choosing a file when title already defined", :js do
      login_as user
      visit send(path, arguments)
    ...
    it "updates loading bar style after invalid file upload", :js do
      login_as user
      visit send(path, arguments)
    ...
    it "updates image cached_attachment field after valid file upload", :js do
      login_as user
      visit send(path, arguments)
    ...
    it "does not update image cached_attachment field after unvalid file upload", :js do
      login_as user
      visit send(path, arguments)
    ...
    it "shows nested image errors after unvalid form submit", :js do
      login_as user
      visit send(path, arguments)
    ...
    it "removes nested image after valid file upload and click on remove button", :js do
      login_as user
      visit send(path, arguments)
    ...
    it "shows successful notice when resource filled correctly without any nested images", :js do
      login_as user
      visit send(path, arguments)
    ...
    it "shows new image after successful creation with one uploaded file", :js do
      login_as user
      visit send(path, arguments)
      send(fill_resource_method_name) if fill_resource_method_name
    ...
    if path.include? "edit"
      ...
      it "does not show add image button when image already exists", :js do
        login_as user
        create(:image, imageable: imageable)
        visit send(path, arguments)
      ...
      it "removes nested field after remove image", :js do
        login_as user
        create(:image, imageable: imageable)
        visit send(path, arguments)
      ...
      it "shows add image button after remove image", :js do
        login_as user
        create(:image, imageable: imageable)
        visit send(path, arguments)
        click_on "Remove image"
