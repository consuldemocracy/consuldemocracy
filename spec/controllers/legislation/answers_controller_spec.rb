...
describe Legislation::AnswersController do
  ...
  describe 'POST create' do
    before do
      ...
      @question_option = create(:legislation_question_option,
        question: @question,
        value: "Yes"
      )
    ...
    it 'creates an ahoy event' do
      ...
      expect(Ahoy::Event.where(
        name: :legislation_answer_created
      ).count).to eq(1)

      expect(Ahoy::Event.last.properties['legislation_answer_id']).to eq(
        Legislation::Answer.last.id
      )
    end
