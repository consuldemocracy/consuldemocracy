...
describe Legislation::AnnotationsController do
  ...
  describe 'POST create' do
    before do
      ...
      @draft_version = create(:legislation_draft_version,
        :published,
        process: @process,
        title: "Version 1"
      )
    ...
    it 'creates an ahoy event' do
      ...
      expect(Ahoy::Event.where(
        name: :legislation_annotation_created
      ).count).to eq(1)

      expect(Ahoy::Event.last.properties['legislation_annotation_id']).to eq(
        Legislation::Annotation.last.id
      )
    end
