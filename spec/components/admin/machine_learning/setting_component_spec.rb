...
RSpec.describe Admin::MachineLearning::SettingComponent, type: :component do
  ...
  it "renders the list of output files when they exist" do
    # Set up the info record
    create(:machine_learning_info, kind: "comments_summary")

    #  Mock the file system check in the MachineLearning module
    allow(MachineLearning).to receive(:data_output_files).and_return(
      { comments_summary: ["summary_2024.csv", "stats_2024.json"] }
    )

    render_inline(Admin::MachineLearning::SettingComponent.new("comments_summary"))

    # Verify the files are listed as links
