...
describe Sensemaker::Backend::Node do
  ...
  describe "#cli_flags" do
    ...
    it "returns structured flags for a vertex runner command" do
      ...
      expect(flags).to include(
        ...
        "inputFile" => job.artefacts.input_path.to_s,
        "outputFile" => backend.send(:output_file)
      )
      expect(flags).not_to have_key("baseUrl")
      expect(flags).not_to have_key("additionalContext")
    end
    ...
    it "includes additionalContext when present" do
      job.update!(additional_context: "Jury brief")
  ...
  describe "#persistable_cli_flags" do
    ...
    it "omits secrets and path keys but keeps adapter metadata" do
      ...
      expect(flags).not_to have_key("apiKey")
      expect(flags).not_to have_key("inputFile")
      expect(flags).not_to have_key("outputFile")
      expect(flags).not_to have_key("additionalContext")
    end
  end
