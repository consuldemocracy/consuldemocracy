class Moderation::Shared::TableComponent < ApplicationComponent
  ...
  def initialize(records, ids_field_name:)
    @records = records
  ...
  private
    ...
    def title_and_link_for(record)
      if record.is_a?(Comment)
        ...
        safe_join([
          commentable.model_name.human,
          link_to(commentable.title, polymorphic_path(commentable), id: dom_id(record, :title))
        ], " - ")
      else
    ...
    def check_box_aria_attributes(record)
      if record.is_a?(Comment)
        { "aria-labelledby": dom_id(record, :title) }
      else
