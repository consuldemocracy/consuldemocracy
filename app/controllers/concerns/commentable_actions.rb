  ...
  def create 
    ...
    if @resource.save_with_captcha
      ...
      if resource_model.to_s== "Proposal"
        redirect_to resource, notice: t('proposals.detail').to_s + t('flash.actions.create.femsinparametro').to_s 
      else
        redirect_to redirect_path, notice: t('flash.actions.create.mas', resource_name: "#{resource_name.capitalize}")
      end
    else
      load_featured_tags
      set_resource_instance
  ...
  def update
    ...
    if resource.save_with_captcha
      if resource_model.to_s== "Proposal"
        ...
        redirect_to resource, notice: t('proposals.detail').to_s + t('flash.actions.update.femsinparametro').to_s
       else
        redirect_to resource, notice: t('flash.actions.update.mas', resource_name: "#{resource_name.capitalize}")
      end
    else
      load_featured_tags
      set_resource_instance
