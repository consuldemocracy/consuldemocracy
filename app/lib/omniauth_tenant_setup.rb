module OmniauthTenantSetup
  class << self
    ...
    def saml(env)
      saml_auth(env, secrets.saml_sp_entity_id, secrets.saml_idp_metadata_url, secrets.saml_idp_sso_service_url)
    end
    ...
    private
      ...
      def saml_auth(env, sp_entity_id, idp_metadata_url, idp_sso_service_url)
        unless Tenant.default?
          ...
          strategy.options[:issuer] = sp_entity_id if strategy.options[:issuer].present? && sp_entity_id.present?

          strategy.options[:idp_metadata] = idp_metadata_url if strategy.options[:idp_metadata].present? && idp_metadata_url.present?
        end
      end
