...
module Sensemaker
  module Backend
    class Node
      ...
      UNESCAPED_RUNNER_KEYS = %w[inputFile outputFile outputBasename].freeze
      ...
      def cli_flags
        ...
        if requires_input? && job.additional_context.present?
          flags["additionalContext"] = job.additional_context.to_s
        end
        if ScriptRegistry.output_flag(job.script) == :output_basename
          flags["outputBasename"] = output_file
        else
          flags["outputFile"] = output_file
        end
        flags
      end
      ...
      private
        ...
        def llm_cli_flags
          ...
          when "vertex"
            ...
            flags["vertexProject"] = runtime_config.vertex_project_id
            flags["vertexLocation"] = runtime_config.vertex_location
          when "openai-compatible"
            flags["adapter"] = "openai-compatible"
            flags["provider"] = runtime_config.compat_provider
            api_key = runtime_config.api_key
            flags["apiKey"] = api_key if api_key.present?
          when "ollama"
            flags["adapter"] = "ollama"
          end

          base_url = runtime_config.base_url
          flags["baseUrl"] = base_url if base_url.present?
          flags
        end

        def format_cli_flag(key, value)
          formatted = if report? || !UNESCAPED_RUNNER_KEYS.include?(key)
