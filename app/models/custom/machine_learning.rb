...
def export_legislation_to_json
  create_data_folder
  filename1 = "/home/deploy/consul/current/public/machine_learning/data/legislation_comments.json"
  filename2 = "/home/deploy/consul/current/public/machine_learning/data/leg_nested_comments.json"

  comments = Comment.where(commentable_type: "Legislation::Question")
  comments_hash = comments.map do |comment|
    question = comment.commentable
    title_translation = I18n.t("legislation.question.title", locale: :en, default: question.title)
    description_translation = I18n.t("legislation.question.description", locale: :en, default: question.description)

    process = question.process
    process_title = I18n.t("legislation.process.title", locale: :en, default: process.title)

    {
      ...
      question_title: title_translation,
      id: comment.id,
      body: comment.body
    }
  end

  # Group comments_hash by type, process_id, process_title, question_id, and question_title
  nested_grouped_comments = {}

  comments_hash.each do |comment|
    type = comment[:type]
    process_id = comment[:process_id]
    process_title = comment[:process_title]
    question_id = comment[:question_id]
    question_title = comment[:question_title]

    # Initialize the nested structure if it doesn't exist
    nested_grouped_comments[type] ||= {}
    nested_grouped_comments[type][process_id] ||= {}
    nested_grouped_comments[type][process_id][process_title] ||= {}
    nested_grouped_comments[type][process_id][process_title][question_id] ||= {
      question_title: question_title,
      comments: []
    }

    # Add the comment to the nested structure
    nested_grouped_comments[type][process_id][process_title][question_id][:comments] << {
      id: comment[:id],
      body: comment[:body]
    }
  end

  # Convert the nested grouped comments to JSON
  nested_grouped_json_output = nested_grouped_comments.to_json

  # Generate JSON and write it to a file
  File.open(filename1, 'w') do |file|
    file.write(comments_hash.to_json)
  end

  File.open(filename2, 'w') do |file|
    file.write(nested_grouped_json_output)
  end
end
