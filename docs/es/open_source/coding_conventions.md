...
Los linters no pueden analizar todas convenciones de código que seguimos. Además, hay código antiguo que no sigue estas convenciones, con lo que a veces te será difícil saber qué camino tomar.
...
Si añades código de CSS o JavaScript que solamente afecta a un componente, crea un archivo siguiente la misma estructura de ficheros que dicho componente. Por ejemplo, el CSS relacionado con el código en `app/components/admin/budgets/form_component.html.erb` está en `app/assets/stylesheets/admin/budgets/form_component.scss`.

Las clases de HTML también siguen una estructura similar; por ejemplo, el archivo `app/components/admin/budgets/form_component.html.erb` contiene un elemento HTML con la clase `budgets-form` (nótese que, sin embargo, no siempre somos consistentes en cuando a cuándo añadir el prefijo `admin-`).
...
```ruby
class Admin::AwesomeThings::IndexComponent do
  include Header
...
* Las personas que usan lectores de pantalla sabrán qué va a pasar al pinchar en el botón va a hacer, pero no tendrán claro qué pasará al pinchar en el enlace.
...
Al usar métodos como `click_link`, asegúrate de que la petición ha terminado antes de generar otra. Conseguir esto en todo momento es muy difícil (nosotros también nos equivocamos en esto a veces), con lo que agracedemos enormemente que tengas esto en mente.
...
Esto a veces resulta en tests que fallan de manera inconsistente (en inglés esto se conoce como "flaky test" o "flaky spec"), es decir, que fallan a veces, pero no siempre. Este tipo de tests resulta muy inconveniente, puesto que, si falla un test durante una _pull request_, vas a tardar un rato en averiguar si el test falla por cambios relacionados con esa _pull request_ o no.
